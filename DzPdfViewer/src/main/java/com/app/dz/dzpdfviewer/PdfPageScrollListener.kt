...
internal class PdfPageScrollListener(
    ...
    private val snapHelper: SnapHelper // ← new parameter
...
*/
    ...
    override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
        ...
        if (snappedPosition != lastDisplayedPage && snappedPosition != RecyclerView.NO_POSITION) {
            updatePage(snappedPosition)
            pageNoTextView.text = pageNoTextView.context.getString(
                R.string.pdfView_page_no, snappedPosition + 1, totalPageCount()
            )
            pageNoTextView.visibility = TextView.VISIBLE
            ...
            lastDisplayedPage = snappedPosition
        }
    }
